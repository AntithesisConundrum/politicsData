...
for i in xrange(1,27):
    ...
    name = precincts[i-1]
    num_to_name[i] = name
    url =  "http://clerk.wabashcounty85.us/custom/00000185/elections/election2018g/Wabash_PrecSumm_"+s+".htm"
    print url
    f = urlopen(url)
    page = f.read()
    pages[i] = page
...
def get_class_name(line):
    """
    go from wabash HTML line to the class of that line
    """
    pattern = "class=(.|_|\\t)+?( |>)"
    res = search(pattern, line)
    if res == None:
        return "None"
    return res.group()[7:-2]

def get_data_from_line(line):
    """
    go from wabash HTML line to the data of that line
    """
    pattern = ">.*<"
    if line == "</div>":
        return "close_div"
...
name = "20181107__in__general__wabash__precinct.csv"
out_f = open(name, "w")
output = writer(out_f)
